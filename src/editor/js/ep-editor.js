(function ($) {
    ...
    var ElementPackEditor = {
        ...
        onPreviewLoaded: function () {
            ...
            var elementorFrontend = previewWindow.elementorFrontend;
            var $preview = previewWindow.jQuery;
            ...
            // Wrapper Link parity in the editor. The frontend injects the overlay
            // anchor server-side (for SEO); the editor renders elements in JS, so
            // the PHP hooks never fire here. Re-create the same overlay client-side
            // for every element (widgets, columns, containers) so it looks and
            // styles identically while editing. `global` fires for all types.
            elementorFrontend.hooks.addAction('frontend/element_ready/global', function ($scope) {
                ElementPackEditor.renderWrapperLink($scope, $preview);
            });
        },

        // Read the wrapper-link setting from an element's model in the editor.
        getWrapperLinkUrl: function (container) {
            ...
            var legacy = read('element_pack_wrapper_link');

            if (legacy && legacy.url) {
                return legacy.url;
            }

            // Atomic widgets store the link under a different key/shape.
            var atomic = read('element_pack_wrapper_link_url');
        ...
        // Inject (or clean up) the overlay anchor for a single element.
        renderWrapperLink: function ($scope, $factory) {
            if (!$scope || !$scope.length || typeof elementor === 'undefined' || !elementor.getContainer) {
                return;
            }

            var jq = $factory || window.jQuery;
            var container = elementor.getContainer($scope.data('id'));

            if (!container) {
                return;
            }

            // Live-update the overlay when the link setting changes, without a full
            // element re-render (the control uses render_type "none"). Bound once.
            if (!container.__bdtWrapperLinkBound) {
                container.__bdtWrapperLinkBound = true;
                container.settings.on(
                    'change:element_pack_wrapper_link change:element_pack_wrapper_link_url',
                    function () {
                        ElementPackEditor.renderWrapperLink($scope, jq);
                    }
                );
            }

            // Always clear our previous artefacts first so edits/removals reflect.
            $scope.children('.bdt-element-link-anchor').remove();
            $scope.off('click.bdtWrapperLink');

            var url = ElementPackEditor.getWrapperLinkUrl(container);

            if (!url) {
                $scope.removeClass('bdt-element-link').css('cursor', '');
                return;
            }

            $scope.addClass('bdt-element-link');

            if ('static' === $scope.css('position')) {
                $scope.css('position', 'relative');
            }

            // Non-blocking overlay: present for markup/CSS parity but never
            // intercepts clicks, so child elements stay selectable while editing.
            jq('<a>', {
                'href': url,
            ...
            // Signal it is linked, and allow testing the link from the editor:
            // Cmd/Ctrl + click opens it in a new tab (a plain click still selects
            // the element for editing).
            $scope.css('cursor', 'pointer');
            $scope.on('click.bdtWrapperLink', function (event) {
